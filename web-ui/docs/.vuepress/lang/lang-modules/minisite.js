...
module.exports = {
    cn: {
        ...
        MIGRATION_DESC: ['随着数字化转型深入，操作系统正在向支持多样性计算、支持全场景的方向发展，在进行操作系统升级时，企业面临以下挑战：硬件兼容性、软件兼容性、升级后系统环境如何快速恢复？openEuler作为一款面向基础设施的开源操作系统，最优支持多样性计算，满足服务器、云、边缘和嵌入式全场景。针对企业在升级操作系统时的需求，推出这份升级指南，助力企业简单、平稳、高效进行操作系统升级。'],
        MIGRATION_LISTCONTENT:[
                {
                    ...
                    CONTENT: [
                        '数字技术日益融入经济社会发展的各个领域和全过程，数字基础设施要面向IT、CT及OT等多种场景，要实现工业互联乃至万物智联，只有建立起强大的技术软件体系，才能够满足数字经济发展的需要；万物互联驱动算力多样性算力(CPU/GPU/XPU等)发展,促使使操作系统进行架构创新；同时随着企业产品持续创新、硬件算力的更新换代、操作系统版本生命周期，操作系统迁移的工作将成为常态；由于操作系统本身的差异带来软件、硬件、配置兼容性问题，影响了操作系统迁移。\n\n',
                        '面向数字化转型过程中，企业一方面需要做好操作系统(如：全场景支持、持续演进、可靠、稳定方面等)选型，支撑企业产品创新、数字化转型；另一方面需要有系统性的迁移方案及工具，保障迁移无忧。'
                    ],
                },
                {
					SUBTITLE: 'openEuler 面向数字基础设施的开源操作系统',
                    LISTTITLE: 'openEuler 是面向的数字基础设施开源操作系统',
                    CONTENT: [

                        'openEuler开源操作系统是面向数字基础设施的开源操作系统，支持服务器、云计算、边缘计算、嵌入式等应用场景，支持多样性计算，致力于提供安全、稳定、易用的操作系统。',

                        '2021年11月9日在工信部的指导及产业力量的协同下，openEuler 社区捐赠给开放原子开源基金会，是当前唯一捐赠到开放原子开源基金会的数字基础设施开源操作系统项目。',

                        '当前国内外14家主流 OSV（麒麟、统信、麒麟信安、SUSE、普华等）均已发布了基于openEuler的操作系统商业发行版；且发行产品已经广泛进入了政府、运营商、金融、电力等多个行业的核心系统，实现了规模商用100+万套，成为企业应用持续创新、稳定运行的首选。',

                        '社区当前已有超过300+企业加入，汇聚了从处理器、整机、基础软件、应用软件、行业客户等全产业链伙伴，100%兼容主流芯片，100%支持主流场景，当前已经有近万名开源贡献者，100多个SIG组，社区维护的软件包达到8000+，是最具活力的开源社区。'

                    ],
                },
                {
                    LISTTITLE:'openEuler聚焦内核能力，释放多样性算力，引领操作系统创新',
                    CONTENT:[
                             'openEuler原生开源，独立演进，从三个层次实现持续发展，助力企业数字化转型。内核创新：在多核调度、多样性算力，以内存中心新架构的支持等方面进行创新，自 2012 年以来向 Linux Kernel 社区持续贡献，在 Linux Kernel 5.10/5.14版本中，内核代码贡献排名第一。核心模块：为用户提供“双平面”运行选择，通过原创 stratoVirt 提供类qemu虚拟化能力，通过原创iSulad 提供类docker容器能力。全功能级：做到独立软件包选型，CVE漏洞管理，bug快速修复 ,主要特性介绍如下：',
                             '1. 性能优化：通过千核调度、内核热升级、NUMA aware、免锁优化、PB级内存扩展等独特内核创新技术，软硬协同优化，最佳支持多样性算力，性能相比于主流OS提升5%~20%。',
                             '2. 云原生“双平面”选择：iSula和StatioVirt虚拟化技术及容器技术，提供从云，到边缘的安全、灵活业务部署，编排能力，应对复杂的业务场景。',
                             '3. 多样性算力：释放强劲算力：通过KAE（Kunpeng Accelerator Engine）库，充分释放鲲鹏硬件算力潜能，提升算力表现。同时通过openssl相结合，在业务零修改情况下，从CPU把加解密算力缷载到硬件加解密卡上， 降底CPU负载。',
                             '4.安全可靠：A-tune 动态智能识別操作系业务场景，匹配场景参数模型，使系统都处在最佳的运行状态，从而提升业务性能。A-ops，从智能定位、配置溯源、架构感知，进行故障定界定位、配置管理、网络拓扑绘制，快速定位问题。']
                }, 
                {
                    LISTTITLE: 'openEuler 架构介绍',
                    CONTENT: [
                        'openEuler是面向数字基础设施的开源操作系统，包括基础加速库、虚拟化、内核、驱动、编译器、OS工具、OpenJDK等组件。创新架构，全栈优化，打造全场景协同的One OS，为多样化架构释放算力。\n\n',
        ...
        MOVE_CONTENT: {
            DESCRIBE: '为了减少操作系统迁移对业务的感知，浙江移动还在容器云底层增加了麒麟欧拉版操作系统的容器集群，并将应用重新发布至创新操作系统集群，实现应用在双平面的统一构建、统一编排、统一调度和统一运行。同时，浙江移动还集成了欧拉容器引擎iSulad，把容器应用的启动速度相比Docker提升了50%以上。',
