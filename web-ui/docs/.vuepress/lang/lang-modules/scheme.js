    ...
    cn: {
        ...
        SCHEME:{
           ...
           TWO_P1: 'openEuler DevKit 面向迁移全流程提供完整工具链及方案，支撑OS迁移平稳、简单、高效，从基础兼容性、兼容性评估、移植适配、搬迁实施四个方面开展；',
            ...
            SIX_P1:'对于原地替换场景，通常与软件部署的形态相关，搬迁的实施方案也会不同，这里主要介绍一般情况下的搬迁：单机、主备和分布式。搬迁实施前，需要提前熟悉软件部署架构及部署组网，针对性输出对应场景的搬迁方案，目前 openEuler 提供主流场景的搬迁方案，如大数据、分布式存储、虚拟化、容器、数据库软件，成功支撑多个行业用户完成搬迁工作。',
